...
namespace Oneiromancer.TMP.Tags
{
    ...
    public class CustomTagPreprocessor : ITextPreprocessor
    {
        ...
        public void AddCustomTagName(string tagName)
        {
            _possibleTags.Add(tagName);
        }
        ...
        private bool ProcessTag(string text, int rawTextIndex, int builderIdx, out int lastIndex)
        {
            ...
            string rawTag = text.Substring(rawTextIndex + 1, lastIndex - rawTextIndex - 1);

            var tagPart = rawTag.Split('=');
            var tag = tagPart[0];
            tag = tag.Trim();
            var tagArgs = new List<string>();
            if (tagPart.Length >= 2)
            {
                var argString = tagPart[1];
                tagArgs.AddRange(argString.Split(','));
            }
            
            if (tag[0] != '/')
            {
                if (!_possibleTags.Contains(tag)) return false;
                _tagInfos.Add(new TagInfo(tag, builderIdx, tagArgs));
            }
            else
            {
                tag = tag.Substring(1);
